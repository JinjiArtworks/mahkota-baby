    ...
    <div class="bg-gray-100 flex-1 p-6 md:mt-16">
        ...
        <div class="card col-span-2 xl:col-span-1">
            <div class="card-header flex justify-between">
                <p class="text-2xl text-black ">Ubah Detail Produk</p>
            </div>
            <div class="p-6">
                <form method="POST"
                    action="{{ route('resources.update', ['id' => $detailsProduct->id]) }} "enctype="multipart/form-data">
                    @csrf
                    {{ method_field('put') }}
                    <div class="mb-4">
                        ...
                        <select name="product_id"
                            ...
                            <option value="{{ $detailsProduct->product->id }}">{{ $detailsProduct->product->nama }}</option>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="bahan" min="1"
                            ...
                            placeholder="Masukkan Bahan Produk" required value="{{ $detailsProduct->bahan }}">
                    </div>
                    <div class="mb-4">
                        <label for="brand" class="block mb-2 text-sm font-medium text-gray-900 ">Brand Produk</label>
                        <input type="text" name="brand" min="1"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  w-full p-3"
                            placeholder="Masukkan Brand Produk" required value="{{ $detailsProduct->brand }}">
                    </div>
                    <div class="mb-4">
                        ...
                        <textarea type="text" name="deskripsi" min="1"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  w-full p-3"
                            placeholder="Masukkan Deskripsi Produk" required>{{ $detailsProduct->deskripsi }}</textarea>
                    </div>
                    <div class="mb-4">
                        ...
                        <textarea type="text" name="kandungan" min="1"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg  w-full p-3"
                            placeholder="Masukkan Kandungan Produk" required>{{ $detailsProduct->kandungan }}</textarea>
                    </div>
                    <button type="submit"
